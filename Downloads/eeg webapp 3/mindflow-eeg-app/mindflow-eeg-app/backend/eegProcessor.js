...
function preprocessSignal(data, sampleRate) {
  ...
  // 3. Simple High-Pass Filter (0.5Hz cutoff) to handle baseline drift
  const cutoff = 0.5; // Hz
  const dt = 1 / sampleRate;
  const RC = 1 / (2 * Math.PI * cutoff);
  const alpha = RC / (RC + dt);

  const filtered = new Array(processed.length);
  for (let i = 0; i < processed.length; i++) {
    if (i === 0) {
      filtered[i] = alpha * (prev_filtered + processed[i] - prev_raw);
    } else {
      filtered[i] = alpha * (filtered[i - 1] + processed[i] - processed[i - 1]);
    }
  }

  // Update persistent state for next window continuity
  prev_raw = processed[processed.length - 1];
  prev_filtered = filtered[filtered.length - 1];
